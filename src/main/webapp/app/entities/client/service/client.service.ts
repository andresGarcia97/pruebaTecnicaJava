...
type RestOf<T extends IClient | NewClient> = Omit<T, 'bornDate' | 'creationDate' | 'lastModificationDate'> & {
...
export class ClientService {
  ...
  protected convertDateFromClient<T extends IClient | NewClient | PartialUpdateClient>(client: T): RestOf<T> {
    return {
      ...client,
      bornDate: client.bornDate?.format(DATE_FORMAT) ?? null,
      creationDate: client.creationDate?.toJSON() ?? null,
      lastModificationDate: client.lastModificationDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restClient: RestClient): IClient {
    return {
      ...restClient,
      bornDate: restClient.bornDate ? dayjs(restClient.bornDate) : undefined,
      creationDate: restClient.creationDate ? dayjs(restClient.creationDate) : undefined,
      lastModificationDate: restClient.lastModificationDate ? dayjs(restClient.lastModificationDate) : undefined,
    };
  }
